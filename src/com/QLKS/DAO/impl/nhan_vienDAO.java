...
//        StringBuilder sql = new StringBuilder("get_by_userName_passWord");
    ...
    public nhan_vienModel findByUserName(String userName) {
        String sql = resourceBundleSQL.getString("get_one_userName");
        List<nhan_vienModel> list = query(sql, new nhan_vienMapper(), userName);
        return list.isEmpty() ? null : list.get(0);
    }
    
    @Override
    public nhan_vienModel findByEmail(String email) {
        String sql = resourceBundleSQL.getString("get_one_email");
        List<nhan_vienModel> list = query(sql, new nhan_vienMapper(), email);
        return list.isEmpty() ? null : list.get(0);
    }
    
    @Override
    public nhan_vienModel findByCmnd(String cmnd) {
        String sql = resourceBundleSQL.getString("get_one_cmnd");
        List<nhan_vienModel> list = query(sql, new nhan_vienMapper(), cmnd);
        return list.isEmpty() ? null : list.get(0);
    }
}
